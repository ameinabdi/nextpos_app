...
import {
  ...
  deleteProduct,
  searchProducts,
} from '../../../../../app/modules/entities/product/product.sagas'
...
test('search success path', () => {
  const response = FixtureAPI.searchProducts()
  const step = stepper(searchProducts(FixtureAPI, '*'))
  // Step 1: Hit the api
  step()
  // Step 2: Successful return and data!
  expect(step(response)).toEqual(put(ProductActions.productSearchSuccess([{ id: 1 }, { id: 2 }])))
})
...
test('search failure path', () => {
  ...
  step()
  // Step 2: Failed response.
  expect(step(response)).toEqual(put(ProductActions.productSearchFailure()))
})
