...
import {
  ...
  deleteCustomer,
  searchCustomers,
} from '../../../../../app/modules/entities/customer/customer.sagas'
...
test('search success path', () => {
  const response = FixtureAPI.searchCustomers()
  const step = stepper(searchCustomers(FixtureAPI, '*'))
  // Step 1: Hit the api
  step()
  // Step 2: Successful return and data!
  expect(step(response)).toEqual(put(CustomerActions.customerSearchSuccess([{ id: 1 }, { id: 2 }])))
})
...
test('search failure path', () => {
  ...
  step()
  // Step 2: Failed response.
  expect(step(response)).toEqual(put(CustomerActions.customerSearchFailure()))
})
