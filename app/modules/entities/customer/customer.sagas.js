...
export function* getCustomers(api, action) {
  ...
  // success?
  if (response.ok) {
    yield put(CustomerActions.customerAllSuccess(response.data))
  } else {
...
export function* searchCustomers(api, action) {
  const { query } = action
  // make the call to the api
  const apiCall = call(api.searchCustomers, query)
  const response = yield call(callApi, apiCall)

  // success?
  if (response.ok) {
    yield put(CustomerActions.customerSearchSuccess(response.data))
  } else {
    yield put(CustomerActions.customerSearchFailure(response.data))
  }
}
